...
logger.info("=" * 80)
logger.info("🚀 CareerGuide Backend Starting...")
logger.info("=" * 80)

try:
    client = AsyncIOMotorClient(mongo_url)
    db = client[db_name]
    logger.info(f"✅ MongoDB Connection Initialized")
...
dsa_topics_collection = db['dsa_topics']
dsa_questions_collection = db['dsa_questions']
dsa_sheets_collection = db['dsa_sheets']
dsa_companies_collection = db['dsa_companies']
...
if gemini_api_key:
    try:
        gemini_generator = GeminiJobGenerator(gemini_api_key)
        article_gemini_generator = GeminiArticleGenerator(gemini_api_key)
        dsa_gemini_generator = GeminiDSAGenerator(gemini_api_key)
        roadmap_gemini_generator = GeminiRoadmapGenerator(gemini_api_key)
        career_tools_handlers = CareerToolsHandlers(db, gemini_api_key)
        logger.info("✅ Gemini AI Connected")
        logger.info(f"   🔑 API Key: {gemini_api_key[:10]}...{gemini_api_key[-4:]}")
        logger.info("   ✨ AI Features: Job Generation, Article Generation, DSA Generation, Roadmap Generation, Career Tools")
    except Exception as e:
        logger.warning(f"⚠️  Gemini AI Initialization Warning: {e}")
        gemini_generator = None
        article_gemini_generator = None
        dsa_gemini_generator = None
        roadmap_gemini_generator = None
        career_tools_handlers = None
else:
...
async def startup_event():
    ...
    try:
        ...
        logger.info(f"   - JWT Secret: {'✅ Configured' if os.environ.get('JWT_SECRET') else '❌ Missing'}")
        logger.info(f"   - JWT Expiration: {os.environ.get('JWT_EXPIRATION', 'Not Set')}")
        ...
        logger.info("=" * 80)
        logger.info("🎉 CareerGuide Backend Started Successfully!")
        logger.info("=" * 80)
        logger.info(f"📖 API Documentation: http://localhost:{os.environ.get('PORT', '8001')}/docs")
        logger.info(f"🔄 Alternative Docs: http://localhost:{os.environ.get('PORT', '8001')}/redoc")
        logger.info("=" * 80)
...
async def shutdown_event():
    ...
    logger.info("=" * 80)
    logger.info("🛑 Shutting Down CareerGuide Backend...")
    logger.info("=" * 80)
    client.close()
...
# =============================================================================
# USER ROUTES - CAREER TOOLS (Auth Required)
# =============================================================================

@api_router.post("/user/career-tools/resume-review", tags=["User - Career Tools"])
async def user_review_resume(request_data: ResumeReviewRequest, current_user = Depends(get_current_user)):
    """Review resume using AI (Auth Required)"""
    if not career_tools_handlers:
        raise HTTPException(status_code=503, detail="Career tools service not available")
    return await career_tools_handlers.review_resume(current_user["id"], request_data.dict())

@api_router.post("/user/career-tools/cover-letter", tags=["User - Career Tools"])
async def user_generate_cover_letter(request_data: CoverLetterRequest, current_user = Depends(get_current_user)):
    """Generate cover letter using AI (Auth Required)"""
    if not career_tools_handlers:
        raise HTTPException(status_code=503, detail="Career tools service not available")
    return await career_tools_handlers.generate_cover_letter(current_user["id"], request_data.dict())

@api_router.post("/user/career-tools/ats-hack", tags=["User - Career Tools"])
async def user_optimize_for_ats(request_data: ATSHackRequest, current_user = Depends(get_current_user)):
    """Optimize resume for ATS using AI (Auth Required)"""
    if not career_tools_handlers:
        raise HTTPException(status_code=503, detail="Career tools service not available")
    return await career_tools_handlers.optimize_for_ats(current_user["id"], request_data.dict())

@api_router.post("/user/career-tools/cold-email", tags=["User - Career Tools"])
async def user_generate_cold_email(request_data: ColdEmailRequest, current_user = Depends(get_current_user)):
    """Generate cold email using AI (Auth Required)"""
    if not career_tools_handlers:
        raise HTTPException(status_code=503, detail="Career tools service not available")
    return await career_tools_handlers.generate_cold_email(current_user["id"], request_data.dict())

@api_router.get("/user/career-tools/my-usage", tags=["User - Career Tools"])
async def user_get_my_usage(current_user = Depends(get_current_user)):
    """Get user's career tools usage history"""
    if not career_tools_handlers:
        raise HTTPException(status_code=503, detail="Career tools service not available")
    return await career_tools_handlers.get_user_usage(current_user["id"])
