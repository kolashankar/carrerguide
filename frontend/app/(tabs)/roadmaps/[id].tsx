...
import Svg, { Circle, Line, Text as SvgText, G, Rect, Defs, Marker, Polygon } from 'react-native-svg';
...
export default function RoadmapDetailScreen() {
  ...
  const renderFlowchart = () => {
    ...
    const maxX = Math.max(...roadmap.nodes.map((n: RoadmapNode) => n.position_x || 0));
    const maxY = Math.max(...roadmap.nodes.map((n: RoadmapNode) => n.position_y || 0));
    
    const svgWidth = Math.max(SCREEN_WIDTH, maxX + 300);
    const svgHeight = Math.max(600, maxY + 300);

    return (
      <Svg width={svgWidth} height={svgHeight}>
        {/* Render connections first (lines) */}
        {roadmap.nodes.map((node: RoadmapNode) => 
          (node.child_nodes || node.connections)?.map((targetId: string) => {
            const targetNode = roadmap.nodes.find((n: RoadmapNode) => n.id === targetId);
            if (!targetNode) return null;

            const isPathCompleted = node.is_completed && targetNode.is_completed;
            ...
            return (
              <Line
                ...
                stroke={isPathCompleted ? '#10b981' : '#e5e7eb'}
                strokeWidth="3"
        ...
        {roadmap.nodes.map((node: RoadmapNode, index: number) => {
          ...
          return (
            <G key={node.id} onPress={() => handleNodePress(node)}>
              ...
              <SvgText
                x={node.position_x}
                y={node.position_y + NODE_RADIUS + 25}
                fontSize="13"
                fill="#1f2937"
                textAnchor="middle"
                fontWeight="700"
              ...
              <SvgText
                x={node.position_x}
                y={node.position_y - NODE_RADIUS - 10}
                fontSize="11"
                fill="#6b7280"
                textAnchor="middle"
                fontWeight="600"
  ...
  if (!roadmap) return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <EmptyState 
        ...
        message="The roadmap you're looking for doesn't exist or has been removed." 
      />
    </SafeAreaView>
  );
