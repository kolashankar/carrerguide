...
// Save article progress (alias for updateReadProgress)
export const saveArticleProgress = async (
  articleId: string,
  progress: number
): Promise<boolean> => {
  return await updateReadProgress(articleId, progress);
};

// Complete article (alias for markAsRead)
export const completeArticle = async (articleId: string): Promise<boolean> => {
  return await markAsRead(articleId);
};
